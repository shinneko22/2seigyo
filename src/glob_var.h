...
float encoder_updated_time;

float distance_r; // 右モーターの累積回転距離 [mm]
float distance_l; // 左モーターの累積回転距離 [mm]

float speed_r; // 右モーターの回転速度 [mm/s]
float speed_l; // 左モーターの回転速度 [mm/s]

float position_x; // ロボットのx座標 [mm] (まだ未使用)
float position_y; // ロボットのy座標 [mm] (まだ未使用)
float direction; // ロボットの方向 [rad] (まだ未使用)

float target_speed; // モーター目標速度 [mm/s]
float target_direction; // 目標方向 [rad]

int r_encoder_incremented_num;
int l_encoder_incremented_num;
